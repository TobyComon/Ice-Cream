...
let choice = [iceCream, vessels, toppings]
let cart = []

function addItem(iceCreamName) {
    console.log(iceCreamName);
    let foundItem = iceCream.find(iceCream => iceCream.name == iceCreamName)
    ...
    iceCream.forEach(iceCream => {
        ...
        <div class="col-md-6 my-2">
            <div class="bg-light card">
                <div>
                <img src="${iceCream.image}" class="rounded" title="${iceCream.name}">
                </div>
                <div class="d-flex p-2">
                    <h6>${iceCream.name}</h6>
                    <h6 class="ms-auto"> ${iceCream.price}</h6>
                </div>
                <div class="d-grid gap-2">
                <button class="btn btn-info" onclick="addItem('${iceCream.name}')">Add To Cart</button>
              </div>
            </div>
        </div>
...
cart.forEach((c) => {
    ...
    template += /*html*/ `
        ...
        <div class="col-12">
            <div class="d-flex text-light align-items-center p-4">
                <h6>${c.name}</h6>
            </div>
        </div>
